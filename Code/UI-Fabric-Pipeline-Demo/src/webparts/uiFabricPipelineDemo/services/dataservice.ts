...
export interface IOpportunityDataService {
  ...
  getOpportunities(web: Web): Promise<Opportunity[]>;
}
...
export class OpportunityDataService implements IOpportunityDataService {
  ...
  public async getOpportunities(web: Web) {
    let returnValue: Opportunity[] = [];
    try {
      let items: Opportunity[] = [];
      let tempItems: any[];
      let clientItems: any[];
      tempItems = await web.lists.getByTitle("Projects").items.select("ID", "Title", "ProjectDescription", "Client/ID", "Client/Title", "EngagementManager/FirstName", "EngagementManager/LastName", "EngagementManager/UserName", "PipelineStatus", "LastContactDate", "NextContactDate", "ContactName", "ContactTitle", "WorkPhone").expand("EngagementManager", "Client").usingCaching().get();
      clientItems = await web.lists.getByTitle("Clients").items.select("Title", "ID", "Company_x0020_Address", "WorkCity", "WorkState", "WorkZip", "WorkPhone").usingCaching().get();
