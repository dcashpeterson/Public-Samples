...
var Loader_resx_1 = tslib_1.__importDefault(require("./Loader.resx"));
var Common_resx_1 = tslib_1.__importDefault(require("../loc/Common.resx"));
var FIRST_RETRY = 1;
...
function loadComponent(manifest, moduleLoader) {
    if (!manifest) {
        return Promise.reject(new Error(sp_core_library_1.Text.format(Loader_resx_1.default.isUndefinedValidateError, 'manifest')));
    }
...
function _loadComponentRetryStrategy(manifest, currentRetryNumber, maxNumberRetries, moduleLoader) {
    if (currentRetryNumber === 1) {
        sp_diagnostics_1._TraceLogger.logVerbose(telemetryConstants.loadComponentLogSource, sp_core_library_1.Text.format(Loader_resx_1.default.loadComponentLog, manifest.id, manifest.alias, manifest.version));
    }
    else {
        sp_diagnostics_1._TraceLogger.logVerbose(telemetryConstants.loadComponentLogSource, sp_core_library_1.Text.format(Loader_resx_1.default.loadComponentRetryLog, manifest.id, manifest.alias, currentRetryNumber, maxNumberRetries));
    }
    return _loadComponentImpl(manifest, moduleLoader)
        .then(function (component) {
        sp_diagnostics_1._TraceLogger.logVerbose(telemetryConstants.loadComponentLogSource, sp_core_library_1.Text.format(Loader_resx_1.default.loadComponentEndLog, manifest.id, manifest.alias, manifest.version));
        return component;
    })
        ...
        if (currentRetryNumber < maxNumberRetries) {
            ...
            sp_diagnostics_1._TraceLogger.logError(telemetryConstants.loadComponentLogSource, new Error(sp_core_library_1.Text.format(Common_resx_1.default.loadComponentMaxRetriesError, manifest.id, manifest.alias, maxNumberRetries)));
            throw error;
        }
    });
}
...
function _validateComponentIsNotEmptyOrThrow(component, manifest) {
    if ((0, sp_lodash_subset_1.isEmpty)(component)) {
        throw ErrorBuilder_1.default.buildLoadComponentReturnsEmptyError(manifest);
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    var defaultObject = component.default;
    if (defaultObject && (0, sp_lodash_subset_1.isEmpty)(defaultObject) && !defaultObject.prototype) {
            ...
            if (resourceManifest) {
                ...
                if (moduleConfiguration_1.failoverPath) {
                    var dep = moduleLoader
                        ...
                        .catch(function (e) {
                        return _processLoadErrors(manifest, name_1, tslib_1.__spreadArray([ResourceUrlChecker_1.default.checkResourceUrl], moduleLoader.loadComponentDependencyErrorProcessors, true), function () {
                            return ErrorBuilder_1.default.buildLoadComponentDependencyFailoverPathError(manifest, name_1, moduleConfiguration_1.failoverPath, e);
                        });
                    });
                    depPromises.push(dep);
                }
...
function _loadPathDependency(manifest, name, loadedPathDependencies, moduleLoader) {
    ...
    sp_diagnostics_1._TraceLogger.logVerbose(telemetryConstants.loadComponentLogSource, sp_core_library_1.Text.format(Loader_resx_1.default.loadPathDependencyLog, name, manifest.id, manifest.alias));
    var resources = manifest.loaderConfig.scriptResources;
...
function _moduleLoaderLoadPathDependency(manifest, name, moduleLoader) {
    ...
    return moduleLoader.load(manifest, name, globalName).catch(function (e) {
        return _processLoadErrors(manifest, name, tslib_1.__spreadArray([ResourceUrlChecker_1.default.checkResourceUrl], moduleLoader.loadPathDependencyErrorProcessors, true), function () { return ErrorBuilder_1.default.buildLoadPathDependencyError(manifest, name, e); });
    });
}
/**
...
function _loadEntryPoint(manifest, moduleLoader) {
    return moduleLoader.load(manifest).catch(function (e) {
        return _processLoadErrors(manifest, manifest.loaderConfig.entryModuleId, tslib_1.__spreadArray([ResourceUrlChecker_1.default.checkResourceUrl], moduleLoader.loadEntryPointErrorProcessors, true), function () { return ErrorBuilder_1.default.buildLoadEntryPointError(manifest, e); });
    });
}
