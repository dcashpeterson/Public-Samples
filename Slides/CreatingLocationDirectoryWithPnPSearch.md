
# Building a Location Directory using PnP Search Web Parts

Thank you for attending my session on Creating a Location Directory using PnP Search Web Parts. I hope you enjoyed the session. Here is a list of the resources that were referenced in the session.

## Source Code

- [Source Code](https://symp.info/LocationDirectory)

## Community Resources

- [PnP Modern Search Web Parts](https://symp.info/PnPModernSearch)
- [PnP Modern Search Templating Guidance](https://microsoft-search.github.io/pnp-modern-search/extensibility/templating/)
- [Using the Modern PnP Search Web Part – Emily Mancini](<https://emilymancini.com/2022/10/26/using-the-modern-pnp-search-web-part/>)
- [What’s a Dev Tenant](https://symp.info/DevTenant)

## Custom Layout Resources

- [Handlebars](https://handlebarsjs.com/)
- [Adaptive Card Designer](https://adaptivecards.io/designer/)

## Contact Information

- [Twitter - @spdcp](https://www.twitter.com/spdcp)
- [LinkedIn - @dcashpeterson](https://www.linkedin.com/in/dcashpeterson/)
